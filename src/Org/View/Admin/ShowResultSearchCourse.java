 ...
 * @author an
 */
public class ShowResultSearchCourse extends javax.swing.JFrame {

    private final Sm_Ctrl controller;

    /**
     * Creates new form ShowResultSearchCourse
     *
     * @param controller
     * @param course
     */
    public ShowResultSearchCourse(Sm_Ctrl controller, Course_Entity course) {
        ...
        jTable2.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (selectedRow >= 0 && SwingUtilities.isLeftMouseButton(e)) {
                    String Name = jTable2.getValueAt(selectedRow, 2).toString();
                    try {

                        Course_Entity course = controller.findCourseByName(Name);

                        if (course != null) {
                            String number = jTable2.getValueAt(selectedRow, 1).toString();
                            String name = jTable2.getValueAt(selectedRow, 2).toString(); // Nom
                            String des = jTable2.getValueAt(selectedRow, 3).toString(); // Prénom
                            String access = jTable2.getValueAt(selectedRow, 4).toString().trim();
                            LocalDate regDate = LocalDate.parse(jTable2.getValueAt(selectedRow, 5).toString()); // Date d'enregistrement
...
//                            String gender = (sex == 1) ? "Masculin" : (sex == 2) ? "Féminin" : "Inconnu";
                            ...
                            String message = String.format(
                                    ...
                                    name, des, access, regDate,teacherName
                            );
                            ...
                            if (access.equals("Autoriser")) {
                                options = new Object[]{"Bloquer", "Modifier", "Supprimer", "Afficher les étudiants", "Annuler"};
                            } else if (access.equals("Supprimer")) {
                                options = new Object[]{"Autoriser", "Modifier", "Restaurer", "Afficher les étudiants", "Annuler"};
                            } else if (access.equals("Bloquer")) {
                                options = new Object[]{"Autoriser", "Modifier", "Supprimer", "Afficher les étudiants", "Annuler"};
                            } else {
                                options = new Object[]{"Annuler"}; // Option par défaut si `statuts` a une valeur inattendue
                            }

                            int option = JOptionPane.showOptionDialog(
                                    ShowResultSearchCourse.this,
                                    message,
                                    "Détails du cours",
                            ...
                            switch (option) {
                                case 0:
                                    if (access == "Autoriser") {
                                        ...
                                        JOptionPane.showMessageDialog(ShowResultSearchCourse.this, "Cours bloqué avec succès !");
                                        Sm_Ctrl contra = new Sm_Ctrl();
                                        dispose();
                                        Dashboard_All_Course AllcourseFrame = new Dashboard_All_Course(contra);
                                        AllcourseFrame.setLocationRelativeTo(null);
                                        AllcourseFrame.setVisible(true);
                                    } else {
                                        controller.AllowCourse(course);
                                        JOptionPane.showMessageDialog(ShowResultSearchCourse.this, "Cours autorisé avec succès !");
                                ...
                                case 1:
                                    ...
                                    Updates_Course UpdateFrame = new Updates_Course(number, name, des, cx);
                                ...
                                case 2:
                                    if (access == "Autoriser" || access == "Bloquer") {
                                        ...
                                        controller.RestoreCourse(course);
                                        JOptionPane.showMessageDialog(ShowResultSearchCourse.this, "Cours réstauré avec succès !");
                                        Sm_Ctrl contrc = new Sm_Ctrl();
                                        dispose();
                                        Dashboard_All_Course AllcourseFrame = new Dashboard_All_Course(contrc);
                                        AllcourseFrame.setLocationRelativeTo(null);
                                        AllcourseFrame.setVisible(true);
                                    }
                                ...
                                case 3:
                                    StudentsListDialog.showDialog(ShowResultSearchCourse.this, course.getId(), controller);
                            ...
                            JOptionPane.showMessageDialog(ShowResultSearchCourse.this, "Erreur : cours sélectionné n'a pas pu être trouvé.", "Erreur", JOptionPane.ERROR_MESSAGE);
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(ShowResultSearchCourse.this, "Erreur lors de la récupération des données : " + ex.getMessage(), "Erreur", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
    ...
    private void handleDeleteCourse(Course_Entity course) {
        ...
        if (success) {
            ...
            Sm_Ctrl contrl = new Sm_Ctrl();
            dispose();
            Dashboard_All_Course AllcourseFrame = new Dashboard_All_Course(contrl);
            AllcourseFrame.setLocationRelativeTo(null);
            AllcourseFrame.setVisible(true);
        } else {
    ...
    public void populateTable(Course_Entity course) {
        DefaultTableModel model = (DefaultTableModel) jTable2.getModel();
        model.setRowCount(0); // Effacer les anciennes données

        if (course == null) {
            String emptyMessage = "Aucun cours trouvé.";
            ...
            int rowNumber = 1;
            String statut = (course.getAccess() == 1) ? "Autoriser" : (course.getAccess() == 2) ? "Bloquer" : (course.getAccess() == 0) ? "Supprimer" : "Inconnu";
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ShowResultSearchCourse.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                Course_Entity cd = new Course_Entity();
                new ShowResultSearchCourse(controllers, cd).setVisible(true);
            }
        });
    }
