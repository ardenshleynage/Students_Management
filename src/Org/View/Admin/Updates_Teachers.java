 ...
 * @author an
 */
public class Updates_Teachers extends javax.swing.JFrame {
    ...
    /**
     * Creates new form Updates_Teachers
     *
     * @param regNumber
     * @param id
     * @param lname
     ...
     * @param controller
     */
    public Updates_Teachers(String regNumber, String id, String lname, String fname, String mail, String d_o_b, String gender, String special, String tel, String pass, Sm_Ctrl controller) {
        initComponents();
        ...
        updateDetails(regNumber, lname, fname, mail, d_o_b, gender, special, tel, pass);
    }

    public static void updateDetails(String regNumber, String lname, String fname, String mail, String d_o_b, String gender, String special, String tel, String pass) {
        if (l_name != null) {
        ...
        if (speciality != null) {
            speciality.setText("6- Spécialité : " + special);
        }
        if (phone != null) {
    ...
    private void showNameEditDialogLastName() {
        ...
        saveButton.addActionListener(event -> {
            ...
            if (!newName.isEmpty()) {
                ...
                Teachers_Entity tea = new Teachers_Entity();

// Mise à jour de l'ID et du nom de famille
                tea.setId(Long.valueOf(id)); // Remplacer 'id' par l'identifiant de l'étudiant
                tea.setLast_name(newName); // Mettre à jour le nom

// Optionnellement, si d'autres champs doivent être modifiés, vous pouvez aussi les ajuster
                try {
                    // Mise à jour via le contrôleur
                    controller.updateLastNameTeachers(tea);
...
// Mise à jour de l'ID et du nom de famille
                tea.setId(Long.valueOf(id)); // Remplacer 'id' par l'identifiant de l'étudiant
                tea.setFirst_name(FirsName); // Mettre à jour le nom

// Optionnellement, si d'autres champs doivent être modifiés, vous pouvez aussi les ajuster
                try {
                    // Mise à jour via le contrôleur
                    controller.updateFirstNameTeachers(tea);
        ...
        saveButton.addActionListener(e -> {
            ...
            if (selectedDate != null) {
                ...
                try {
                    ...
                    tea.setDob(localDate);

                    // Mise à jour via le contrôleur
                    controller.updateDobTeachers(tea);
...
//----------------------------------------------     
    private void showEditDialogSex() {
        ...
        validateButton.addActionListener(e -> {
            ...
            Teachers_Entity tea = new Teachers_Entity();
            tea.setId(Long.valueOf(id)); // Remplace 'id' par l'identifiant de l'étudiant
            tea.setSex(newSex); // Mise à jour du sexe

            try {
                // Mise à jour via le contrôleur
                controller.updateSexTeachers(tea);
    ...
    private void showNameEditDialogEmail() {
        ...
        saveButton.addActionListener(event -> {
            ...
            if (!newEmail.isEmpty()) {
                ...
                Teachers_Entity tea = new Teachers_Entity();
                tea.setId(Long.valueOf(id)); // Remplacer 'id' par l'identifiant de l'étudiant
                tea.setEmail(newEmail); // Mettre à jour l'email

                try {
                    // Tenter la mise à jour de l'e-mail via le contrôleur
                    boolean success = controller.updateEmailTeachers(tea);
...
//----------------------------------------------
//----------------------------------------------    
    private void showNameEditDialogSpeciality() {
        JDialog dialog = new JDialog((Frame) null, "Modifier la spécialité", true);
        dialog.setSize(400, 250);
        dialog.setLayout(new BorderLayout(10, 10)); // Ajoute un espacement entre les panels
        dialog.setLocationRelativeTo(null);
        ...
        // Panel pour afficher "Nom Actuel" centré en haut
        JPanel titlePanel = new JPanel();
        JLabel label = new JLabel("Spécialité actuel : " + special);
        label.setFont(new java.awt.Font("sansserif", Font.BOLD, 20));
        label.setForeground(new java.awt.Color(0, 0, 204));
        titlePanel.add(label);
        ...
        // Panel pour "Nouveau Nom : " + JTextField (alignés horizontalement)
        JPanel inputPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 10));
        JLabel label2 = new JLabel("Nouvelle spécialité : ");
        label2.setFont(new java.awt.Font("sansserif", Font.BOLD, 18));
        JTextField nameField = new JTextField(15);
        inputPanel.add(label2);
        inputPanel.add(nameField);
        ...
        // Action pour le bouton "Envoyer"
        saveButton.addActionListener(event -> {
            String newspeciality = nameField.getText().trim();
...
// Mise à jour de l'ID et du nom de famille
                tea.setId(Long.valueOf(id)); // Remplacer 'id' par l'identifiant de l'étudiant
                tea.setSpeciality(newspeciality);

// Optionnellement, si d'autres champs doivent être modifiés, vous pouvez aussi les ajuster
                try {
                    // Mise à jour via le contrôleur
                    controller.updateSpecialityTeachers(tea);
                    JOptionPane.showMessageDialog(dialog, "Spécialité modifié avec succès !");

                    // Mettre à jour le label avec le nouveau nom
                    speciality.setText("6- Spécialité : " + newspeciality);

                    // Ferme la fenêtre pop-up
                    dialog.dispose();

                    // Actualiser la page mère
                    refreshMainFrameSpeciality();
                } catch (Exception e) {
                    // En cas d'erreur, afficher un message d'erreur
                    JOptionPane.showMessageDialog(dialog, "Erreur lors de la mise à jour de la spécialité : " + e.getMessage(), "Erreur", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                // Si le nom est vide, afficher une erreur
                JOptionPane.showMessageDialog(dialog, "La spécialité ne peut pas être vide.", "Erreur", JOptionPane.ERROR_MESSAGE);
            }
...
// Méthode pour actualiser la page mère
    private void refreshMainFrameSpeciality() {
        SwingUtilities.getWindowAncestor(speciality).repaint();
    }
    ...
    private void showEditDialogPhone() {
        ...
        saveButton.addActionListener(event -> {
            ...
            if (!phone_input.isEmpty()) {
                ...
                Teachers_Entity tea = new Teachers_Entity();
                tea.setId(Long.valueOf(id)); // Remplacer 'id' par l'identifiant de l'étudiant
                tea.setPhone(newphone); // Mettre à jour l'email

                try {
                    // Tenter la mise à jour de l'e-mail via le contrôleur
                    boolean success = controller.updatePhoneTeachers(tea);
    ...
    private void showEditDialogPassword() {
        ...
        confirmButton.addActionListener(e -> {
            ...
            if (hashedPassword != null) {
                ...
                Teachers_Entity tea = new Teachers_Entity();
                tea.setId(Long.valueOf(id));
                tea.setPassword(hashedPassword); // Mise à jour du mot de passe

                try {
                    // Mise à jour via le contrôleur
                    controller.updatePasswordTeachers(tea);
...
//----------------------------------------------    
    public void showEditDialogRegNumb() {
        ...
        String newRegNumber = controller.generateUniqueRegNumberTeachers();
        ...
        saveButton.addActionListener(event -> {
            try {
                Teachers_Entity tea = new Teachers_Entity();
                tea.setId(Long.valueOf(id)); // Assurez-vous que l'ID est valide
                tea.setReg_number(newRegNumber);

                boolean success = controller.updateRegNumberTeachers(tea);
    ...
    private void initComponents() {
        ...
        speciality.setText("6- Spécialité :");
        speciality.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (speciality != null) {
                    showNameEditDialogSpeciality();
                }

            }
            @Override
            public void mouseEntered(MouseEvent e) {
                speciality.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
                speciality.setForeground(new java.awt.Color(0, 0, 0));

            }

            @Override
            public void mouseExited(MouseEvent e) {
                speciality.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
                speciality.setForeground(new java.awt.Color(0, 0, 204));
            }
        });
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        dispose();
        Dashboard_All_Teachers AllTeachersFrame = new Dashboard_All_Teachers(ctx);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Updates_Teachers.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new Updates_Teachers("", "", "", "", "", "", "", "", "", "", xt).setVisible(true);
            }
        });
    }
