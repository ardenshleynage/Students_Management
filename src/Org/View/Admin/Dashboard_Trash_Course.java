 ...
 * @author an
 */
public class Dashboard_Trash_Course extends javax.swing.JFrame {

    private final Sm_Ctrl controller;

    /**
     * Creates new form Dashboard_Trash_Course
     *
     * @param controller
     */
    public Dashboard_Trash_Course(Sm_Ctrl controller) {
    ...
    public void populateTable() {
        boolean NoCourses = controller.NoTrashCourse();
        ...
        if (NoCourses) {
            ...
            List<Course_Entity> CoursesList = controller.getAllTrashCourse();  // Fetch all people from the controller

            // Clear the existing rows
            int rowNumber = 1;

            for (Course_Entity course : CoursesList) {
        ...
        jTable2.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (selectedRow >= 0 && SwingUtilities.isLeftMouseButton(e)) {
                    try {
                        List<Course_Entity> Courses = controller.getAllTrashCourse();
                        ...
                        if (course != null) {
                            ...
                            switch (option) {
                                case 0:
                                    EraseCourseQuestion(course);
                                    break;
                                case 1:
                                    controller.RestoreCourse(course);
                                    JOptionPane.showMessageDialog(Dashboard_Trash_Course.this, "Cours réstauré avec succès !");
                                    populateTable();
                                    break;
                                case 2:
                                    break;
                            ...
                            JOptionPane.showMessageDialog(Dashboard_Trash_Course.this, "Erreur : cours sélectionné n'a pas pu être trouvé.", "Erreur", JOptionPane.ERROR_MESSAGE);
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(Dashboard_Trash_Course.this, "Erreur lors de la récupération des données : " + ex.getMessage(), "Erreur", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
    ...
    private void EraseCourseQuestion(Course_Entity course) {
        // Show a dialog with Yes and No options
        int option = JOptionPane.showOptionDialog(
                null,
                "Voulez-vous réellement supprimez définitivement le cours ?",
                "Confirmation",
    ...
    private void handleEraseCourse(Course_Entity course) {
        boolean success = controller.SureEraseCourse(course);

        if (success) {
            JOptionPane.showMessageDialog(this, "Le cours  a été supprimés définitivement.", "Information", JOptionPane.INFORMATION_MESSAGE);
            populateTable();
        } else {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Dashboard_Trash_Course.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new Dashboard_Trash_Course(controllers).setVisible(true);
            }
        });
    }
